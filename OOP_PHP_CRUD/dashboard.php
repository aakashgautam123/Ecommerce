	 			    ...
	 			    foreach($products as $product)
	 			    {
	 					...
	 					{		
	 						...
	 						if(isset($_POST['filter']))
	 						{
	 							...
	 							$sql.= " ORDER BY $column_name DESC";
	 						}

	 						$result = $orders->link->query($sql);
	 						
	 						while($row = mysqli_fetch_array($result))
	 						{
	 							array_push($totalorders,$row);
	 						}
	 						...
	 						?>		<h2 style="font-family:helvetica;color:brown;">Filter By</h2>
	 									<form action ="dashboard.php?parameter=vieworders" method="POST">
	 								<select name="filter" style="display:inline-block;">
	 									<option value="product_date">Latest Date</option>
	 									<option value="subtotal_price">Maximum Order Price</option>
	 									
	 								</select>
	 								<input type="submit" name="filterorder" value="Filter" style="display:inline;">
	 							</form>
	 							<table style="text-align:justify-left;padding:10px;">	
	 								<tr style="margin-right:10px;background-color: black;color:white;height:50px;">
	 									<th>Order ID</th>
	 									<th>User ID</th>
	 									<th>Product ID</th>
...
elseif(isset($_GET['cancelorder']))
{
	...
	$script = "<script>window.location.href='dashboard.php?parameter=vieworders';</script>";
